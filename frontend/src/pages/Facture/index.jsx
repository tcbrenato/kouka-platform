...
export default function Facture() {
  ...
  async function sauvegarder() {
    ...
    setSaving(true)
    const d = new Date()
    const numero = `FAC-${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}-${String(Math.floor(Math.random()*1000)).padStart(3,'0')}`
    ...
    if (!error) { setMessage('✅ Facture sauvegardée !'); setTimeout(() => setMessage(''), 3000) }
    setSaving(false)
  }

  function telechargerPDF() {
    const d = new Date()
    const nom = `Facture-${client.nom || 'client'}-${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}.pdf`
      ...
      <div className="no-print space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-navy-dark">Facture Express</h2>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-md overflow-hidden">
          <div className="bg-navy-dark text-white px-4 py-3">
            <span className="text-sm font-semibold">Produits / Services</span>
          </div>
